...
| TAC-07 | engine | ミドルブロックの厚みが 18–35m、ライン間距離が 6–16m に収まる | 未充足（#137） | shape.test.mjs 「shape-gate v1: 全試合の形状帯」 |
...
| UX-12 | ux | 複数の PK 戦をまたいで選手ごとに本数を数える（順位付けをしない・端末内のみ） | 担保 | live.test.mjs 「蓄積: 複数の PK 戦をまたいで選手ごとに合算する」<br>live.test.mjs 「蓄積: 同一性はチーム名＋選手名」<br>uicontract.test.mjs 「端末内へ書くキーは登録済みのものだけ」 |
