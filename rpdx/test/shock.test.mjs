...
test("#80 検証: 未知kind拒否・時刻外拒否・チーム毎3件目拒否", () => {
  ...
  sc = S.withShockGoal(MATCH, sc, { t: 1200, team: "JPN", kind: "deflection" }).scenario;
  sc = S.withShockGoal(MATCH, sc, { t: 2000, team: "JPN", kind: "deflection" }).scenario;
  const r3 = S.withShockGoal(MATCH, sc, { t: 3000, team: "JPN", kind: "deflection" });
  assert.ok(!r3.validation.ok, "3件目拒否");
});
