...
(() => {
  ...
  const chainBall = (match, scenario, t) => {
    ...
    let travelT = 0;
    if (s.from) {
      const fromSeg = { team: s.from.team, no: s.from.no, slot: s.from.slot };
      const a0 = holderPos(fromSeg, s.t0);
      const b0 = s.restart ? { x: s.rx, y: s.ry } : holderPos(s, s.t0);
      const vcap = s.restart ? 12 : 24;
      travelT = Math.max(s.tf - s.t0, Math.hypot(b0.x - a0.x, b0.y - a0.y) / vcap, 0.001);
    }
    const inTransit = s.from && (t - s.t0) < travelT;
    if (!inTransit) {
      // 到達済み: リスタートはピン静止 → 保持者の足元へスムーズ復帰。
      // 解放ブレンドはセグメント終端までに必ず完了させる（#50 — 次セグメントは
      // 保持者の足元から発進するため、未完了だと瞬間移動になる）
      if (s.restart && t >= s.tf) {
        const relT = Math.max(0.25, Math.min(1.2, (s.t1 ?? Infinity) - (s.tf + s.rdelay)));
        const uOut = N.smooth(clamp((t - (s.tf + s.rdelay)) / relT));
        if (uOut < 1) {
    ...
    const u = N.smooth(clamp((t - s.t0) / travelT));
    const a = holderPos({ team: s.from.team, no: s.from.no, slot: s.from.slot }, t);
    const b = (s.restart && t <= s.tf + s.rdelay) ? { x: s.rx, y: s.ry } : holderPos(s, t);
  ...
  E.clearCaches = () => {
    ...
    rankCache.clear(); lineCache.clear();
    stateMemo = null;
  };
})();
